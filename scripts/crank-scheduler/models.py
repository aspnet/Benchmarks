...
class MachineCapability:
    ...
    # Which profile to use by default (defaults to first profile)
    default_profile: Optional[str] = None

    def __post_init__(self):
        if not self.profiles:
            raise ValueError(
                f"At least one profile must be specified for machine type {self.machine_type}")
        ...
        if self.default_profile not in self.profiles:
            raise ValueError(
                f"Default profile '{self.default_profile}' not in available profiles: {self.profiles}")
...
class Machine:
    ...
    name: str
    # Direct mapping for efficient access
    capabilities: Dict[MachineType, MachineCapability]
    # Ordered list of preferred partner machines
    preferred_partners: List[str] = field(default_factory=list)
    # Machine group for compatibility filtering
...
class Scenario:
    ...
    # List of target machine names (e.g., ["gold-lin", "gold-win"])
    ...
    # Machine-specific profile overrides
    profile_overrides: Optional[Dict[str, Dict[MachineType, str]]] = None

    def __post_init__(self):
        # Validate that we have target machines
        if not self.target_machines:
            raise ValueError(
                f"Scenario {self.name} must have at least one target machine")
...
class MachineAssignment:
    ...
    scenario: Scenario
    # Map of machine type to assigned machine
    machines: Dict[MachineType, Machine]
    ...
    def calculate_duration(self) -> float:
        ...
        for assignment in self.assignments:
            if assignment.scenario.estimated_runtime:
                max_runtime = max(
                    max_runtime, assignment.scenario.estimated_runtime)
...
class Schedule:
    ...
    def get_machine_utilization(self) -> Dict[str, float]:
        ...
        for machine_name, usage_time in machine_usage.items():
            utilization[machine_name] = (
                usage_time / self.total_estimated_time) * 100
...
class ConfigurationMetadata:
    ...
    # If True, machines can only work with others in the same group
    enforce_machine_groups: bool = True
    ...
    def __post_init__(self):
        if self.total_estimated_time is None:
            self.total_estimated_time = sum(
                stage.estimated_duration or 0 for stage in self.stages)

    def add_stage(self, stage: Stage):
        """Add a stage to this partial schedule"""
        self.stages.append(stage)
        self.total_estimated_time = (
            self.total_estimated_time or 0) + (stage.estimated_duration or 0)
