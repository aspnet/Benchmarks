...
from models import (CombinedConfiguration, Machine, MachineType, Scenario,
                    ScenarioType)
from schedule_operations import ScheduleOperations
from schedule_splitter import MultiYamlSummary, ScheduleSplitter
...
Examples:
  ...
  python main.py --config config.json --format table
    ...
    input_group.add_argument('-c', '--config', type=str,
                             help='Path to combined configuration file (machines + scenarios + metadata) (JSON)')

    # Output options
    output_group = parser.add_argument_group('Output Options')
    output_group.add_argument('--summary-output', type=str,
                              help='Summary Output file path (default: print to console)')
    output_group.add_argument(
        '--yaml-output', type=str, help='Output directory for YAML files (default: current directory)')
    ...
    print(
        f"Creating schedule for {len(scenarios)} scenarios on {len(machines)} machines...")
    ...
    if args.template:
        yamls_generated = ScheduleOperations.process_yaml_generation(
            args, partial_schedules, config)

        # Print summary
        summary = MultiYamlSummary(yamls_generated, len(
            config.scenarios), partial_schedules)
        summary.print_summary()
        summary.print_execution_plans()

        if not yamls_generated:
            raise RuntimeError(
                "No YAML files were generated. Check your template and configuration.")
    else:
...
def list_scheduled_jobs(scenarios: List[Scenario], machines: List[Machine]):
    ...
    for scenario in scenarios:
        ...
        for target_machine in scenario.target_machines:
            ...
            if target_machine not in machine_map:
                print(
                    f"WARNING: Target machine '{target_machine}' not found in machine list")
                continue
            ...
            machine_types = [
                mt.value for mt in scenario.get_required_machine_types()]
    ...
    for scenario_name, jobs in jobs_by_scenario.items():
        ...
        print(
            f"  Target machines: {', '.join([job['target_machine'] for job in jobs])}")
        ...
        for job in jobs:
            print(
                f"    → {job['name']} ({job['scenario_type']} machines, {job['estimated_runtime']})")
        print()
    ...
    for machine in machines:
        ...
        if machine.capabilities:
            primary_type = min(machine.capabilities.keys(),
                               key=lambda mt, m=machine: m.capabilities[mt].priority)
    ...
    for machine_type, machine_names in machines_by_type.items():
        print(
            f"{machine_type.upper()} machines ({len(machine_names)}): {', '.join(machine_names)}")
    ...
    single_machine_jobs = sum(
        1 for scenario in scenarios for _ in scenario.target_machines if scenario.scenario_type == ScenarioType.SINGLE)
    dual_machine_jobs = sum(
        1 for scenario in scenarios for _ in scenario.target_machines if scenario.scenario_type == ScenarioType.DUAL)
    triple_machine_jobs = sum(
        1 for scenario in scenarios for _ in scenario.target_machines if scenario.scenario_type == ScenarioType.TRIPLE)
    ...
    sut_machines = len(
        [m for m in machines if MachineType.SUT in m.capabilities])
    load_machines = len(
        [m for m in machines if MachineType.LOAD in m.capabilities])
    db_machines = len(
        [m for m in machines if MachineType.DB in m.capabilities])
    ...
    if dual_machine_jobs > 0 and load_machines == 0:
        print(
            "⚠️  WARNING: Dual-machine jobs require load machines, but none are available!")
...
def list_jobs_by_machine(scenarios: List[Scenario], machines: List[Machine]):
    ...
    sorted_machines = sorted(machines, key=lambda m: (
        get_primary_machine_type(m), m.name))
    filtered_machines = [
        m for m in sorted_machines if MachineType.SUT in m.capabilities]
    # Display jobs for each machine
    for machine in filtered_machines:
        machine_jobs = jobs_by_machine[machine.name]
        total_runtime = sum(_extract_runtime_minutes(job['estimated_runtime'])
        ...
        if not machine_jobs:
            ...
            sorted_jobs = sorted(
                machine_jobs, key=lambda j: j['scenario_name'])
            for job in sorted_jobs:
                scenario_types_str = ", ".join(job['machine_types'])
                print(
                    f"    → {job['name']} ({scenario_types_str}, {job['estimated_runtime']})")
        print()
    ...
    if machine_type in machines_by_type:
        avg_runtime = runtime_by_type[machine_type] / \
            machines_by_type[machine_type] if machines_by_type[machine_type] > 0 else 0
