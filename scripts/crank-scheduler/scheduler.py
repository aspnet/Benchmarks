...
import statistics
from models import (
    Machine, Scenario, MachineAssignment, Stage, Schedule,
    MachineType, ScenarioType, CombinedConfiguration, MachineCapability
)
...
class MachineAllocator:
    ...
    def find_machine_assignment(self, scenario: Scenario, target_machine: str, used_machines: Set[str]) -> Optional[Tuple[Dict[MachineType, Machine], Dict[MachineType, str]]]:
        ...
        for machine_type in required_types:
            if machine_type == MachineType.SUT:
                ...
                machine_profile = self._select_exact_sut_machine(target_machine, used_machines_temp, scenario)
        ...
        for machine in self.machines.values():
            ...
            # High priority for preferred partners (LOAD/DB case)
                ...
                try:
                    ...
                    priority = 0.1 + (partner_index * 0.1)  # 0.1, 0.2, 0.3, etc.
                except ValueError:
    ...
    def _select_profile_for_capability(self, capability: MachineCapability, machine: Machine, 
        ...
        if scenario:
            preferred_profile = scenario.get_preferred_profile(machine.name, machine_type)
    ...
    def _select_exact_sut_machine(self, target_machine: str, used_machines: Set[str], 
        ...
        # Select profile based on scenario preferences or default
        profile = self._select_profile_for_capability(capability, machine, MachineType.SUT, scenario)
        return (machine, profile)


class RuntimeEstimator:
    """Estimates runtimes for scenarios based on similar scenarios"""

    def __init__(self, scenarios: List[Scenario]):
        self.scenarios = scenarios
        # Group known runtimes by scenario name (not parsed scenario name)
        self._scenario_runtimes = defaultdict(list)
        for scenario in scenarios:
            if scenario.estimated_runtime is not None:
                self._scenario_runtimes[scenario.name].append(
                    scenario.estimated_runtime)

    def estimate_runtime(self, scenario: Scenario) -> float:
        ...
        if scenario_type in self._scenario_runtimes:
            return statistics.mean(self._scenario_runtimes[scenario_type])
...
class CrankScheduler:
    ...
    def __init__(self, machines: List[Machine], scenarios: List[Scenario], config: CombinedConfiguration):
        ...
        for scenario in self.scenarios:
            if scenario.estimated_runtime is None:
                scenario.estimated_runtime = self.estimator.estimate_runtime(
                    scenario)
    ...
    def _create_stage(self, stage_id: int, available_runs: List[Tuple]) -> Stage:
        ...
        assignments = []
        used_machines = set()
        queue_assignments = [[] for _ in range(self.max_queues)]  # Track assignments per queue
