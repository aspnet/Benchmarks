...
from models import Schedule, PartialSchedule, CombinedConfiguration
...
class ScheduleSplitter:
    ...
    def _split_by_balanced_runtime(self, schedule: Schedule) -> List[PartialSchedule]:
        ...
        stages_with_runtime = [(stage, stage.estimated_duration or 0) for stage in schedule.stages]
        stages_with_runtime.sort(key=lambda x: x[1], reverse=True)
        
        # Initialize bins (partial schedules)
        bins = [PartialSchedule(f"part_{i+1:02d}", []) for i in range(target_count)]
    ...
    def generate_schedules(self, base_schedule: str) -> List[str]:
        ...
        for i in range(target_count):
            if i == 0:
                ...
                offset_schedule = self._offset_cron_schedule(base_schedule, i * offset_hours)
...
class MultiYamlSummary:
    ...
    def __init__(self, yaml_files: List[dict], total_scenarios: int, partial_schedules: Optional[List[PartialSchedule]] = None):
    ...
    def print_summary(self):
        ...
        balance_ratio = (max_runtime - min_runtime) / avg_runtime * 100 if avg_runtime > 0 else 0
        
        print(f"Runtime balance:")
    ...
    def print_execution_plans(self):
        ...
        for i, (yaml_info, partial_schedule) in enumerate(zip(self.yaml_files, self.partial_schedules)):
            print("\n" + "=" * 80)
            print(f"EXECUTION PLAN FOR {yaml_info['file'].upper()}")
            print("=" * 80)
            print(f"Schedule: {yaml_info['schedule']}")
            print(f"Estimated Runtime: {yaml_info['estimated_runtime']:.1f} minutes")
            print(f"Stages: {yaml_info['stage_count']}")
            print()
            ...
            execution_plan = ScheduleExporter.to_summary_table(display_schedule)
