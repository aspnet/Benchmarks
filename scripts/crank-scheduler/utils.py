...
import yaml
from models import (CombinedConfiguration, ConfigurationMetadata, Machine,
                    MachineCapability, MachineType, Scenario, ScenarioType,
                    Schedule, YamlGenerationConfig)
    ...
    def load_combined_configuration(file_path: str) -> CombinedConfiguration:
        ...
        if yaml_gen_data:
            yaml_generation = YamlGenerationConfig(
                ...
                schedule_offset_hours=yaml_gen_data.get(
        ...
        metadata = ConfigurationMetadata(
            ...
            enforce_machine_groups=metadata_data.get(
        ...
        for machine_data in data.get('machines', []):
            ...
            for machine_type_str, cap_data in machine_data.get('capabilities', {}).items():
                ...
                capability = MachineCapability(
                    ...
                    profiles=profiles,
                    # Can be None, will be set to first profile in __post_init__
...
class ScheduleExporter:
    ...
    def to_summary_table(schedule: Schedule) -> str:
        ...
        lines.append("=" * 80)
        lines.append(
            f"Total Estimated Time: {schedule.total_estimated_time:.1f} minutes")
        ...
        for stage in schedule.stages:
            ...
            lines.append(
                "  Queue | Scenario                    | Runtime (min) | Machines")
            lines.append(
                "  ------|-----------------------------|--------------|---------")
            ...
            for assignment in stage.assignments:
                ...
                if sut_machine is None:
                    raise ValueError(
                        f"No SUT machine assigned for scenario: {assignment.scenario.name}")
                ...
                lines.append(
                    f"  {assignment.queue_id:4d}  | {scenario_with_machine:27s} | {runtime:12.1f} | {machine_list}")
