...
from models import (CombinedConfiguration, MachineAssignment, MachineType,
                    PartialSchedule, YamlGenerationConfig)
...
class TemplateDataGenerator:
    ...
    def _create_job_from_assignment(self, assignment: MachineAssignment) -> Optional[Dict[str, Any]]:
        ...
        target_machine_name = assignment.target_machine.replace('-', ' ').title()
        ...
        for machine_type, machine in assignment.machines.items():
            profile_name = assignment.get_profile_for_machine_type(machine_type)
            if profile_name:
