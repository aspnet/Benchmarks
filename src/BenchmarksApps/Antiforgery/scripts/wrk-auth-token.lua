...
response = function(status, headers, body) -- after each response https://github.com/wg/wrk/blob/a211dd5a7050b1f9e8a9870b95513060e72ac4a0/SCRIPTING#L43
   if not token and status == 200 then
      ...
      nameWithValue = string.match(destination_number, "(.-);")
      token = string.sub(nameWithValue, string.len(antiforgeryTokenHeaderName), string.len(nameWithValue) - string.len(antiforgeryTokenHeaderName) + 1)
