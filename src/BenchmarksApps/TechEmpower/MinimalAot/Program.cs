...
app.MapGet("/json", () => new JsonMessage("Hello, World!"));

app.MapGet("/json/result", () => Results.Ok(new JsonMessage("Hello, World!")));

app.MapGet("/db", async (Db db) => await db.LoadSingleQueryRow());

app.MapGet("/db/result", async (Db db) => Results.Ok(await db.LoadSingleQueryRow()));
...
app.MapGet("/queries/{count}/result", async (Db db, int count) => Results.Ok(await db.LoadMultipleQueriesRows(count)));

app.MapGet("/updates/{count}", async (Db db, int count) => await db.LoadMultipleUpdatesRows(count));

app.MapGet("/updates/{count}/result", async (Db db, int count) => Results.Ok(await db.LoadMultipleUpdatesRows(count)));
