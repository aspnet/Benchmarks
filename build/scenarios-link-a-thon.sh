...
jobs=(
  ...
  "-n LinkAThonTrimmedAndR2RSingleFileWithTrimListCrossgen2 $trend $linkAThonJobs --linux-only" 
...
do
    ...
    do
        ...
        dotnet $ROOT/.build/BenchmarksDriver/BenchmarksDriver.dll -s $s -c $BENCHMARKS_CLIENT $job --session $SESSION -q "$BENCHMARKS_SQL" $BENCHMARKS_ARGS --sdk 3.0.100 --aspnetcoreversion 3.0 --runtimeversion 3.0 --collect-counters
        # error code in $?
    done
done
