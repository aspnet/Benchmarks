...
do
    ...
    do
        ...
        do
            ...
            dotnet $ROOT/.build/BenchmarksDriver/BenchmarksDriver.dll -s $s -c $BENCHMARKS_CLIENT $job $baseline -i 5 -x 1 --duration 10 --warmup 5 -q "$BENCHMARKS_SQL" $BENCHMARKS_ARGS
            # error code in $?
        done
    done
done
