...
do
    ...
    do
        ...
        do
            ...
            dotnet $ROOT/.build/BenchmarksDriver/BenchmarksDriver.dll -s $s -c $BENCHMARKS_CLIENT $job $baseline -i 1 --duration 20 --warmup 20 --quiet --session $SESSION -q "$BENCHMARKS_SQL" --table AspNetTieredComp $BENCHMARKS_ARGS --sdk 3.0.100
            # error code in $?
        done
    done
done
