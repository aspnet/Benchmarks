...
jobs=(
  ...
  # GRPC ASP.NET Core
  "-n GrpcUnary-h2load --webHost KestrelSockets $trend $grpcManagedJobs --connections 64 --warmup 5 -m h2c --no-startup-latency"
  "-n GrpcUnary-h2load --webHost KestrelSockets $trend $grpcManagedJobs --connections 64 --warmup 5 -m h2 --no-startup-latency"
  ...
  # GRPC C-core
  "-n GrpcUnary-h2load --webHost CCore $trend $grpcNativeJobs --connections 64 --warmup 5 -m h2c --no-startup-latency"
...
do
    ...
    do
        ...
        dotnet $ROOT/.build/BenchmarksDriver/BenchmarksDriver.dll -s $s -c $BENCHMARKS_CLIENT $job --session $SESSION -q "$BENCHMARKS_SQL" $BENCHMARKS_ARGS --sdk 3.0.100-rc2-014229 --self-contained --collect-counters
        # error code in $?
    done
done
